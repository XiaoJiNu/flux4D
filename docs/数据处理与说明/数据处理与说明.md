...
需要注意的是：epoch 秒级时间戳如果直接写入 `float32`，会因为精度不足导致相邻帧的
`Δt≈0`（例如 10Hz 的 0.1s 间隔会被抹平），从而让后续的时间推进/运动学习失效。

因此，本项目在 Lift 初始化阶段使用 **LiDAR 相对时间（秒）**：

- 对当前 clip，取 `t0 = timestamps.lidar[0]`。
- 每个帧的时间使用 `t_rel[k] = timestamps.lidar[k] - t0`（范围约 0~1.5s）。
- `t_rel` 再写入高斯的 `timestamps`（`float32`）用于后续时间推进。
