...
— 实现方案（对齐正文 3.4 + 补充材料 A.1/A.2/A.3）

> 本阶段从 Flux4D-base 升级为论文最终模型 Flux4D（正文 3.4：Improving Realism and Flow）。
> 关键增强包括：
>
> 1) **Iterative refinement（N=3）**：利用渲染损失对 3D Gaussians 的梯度反馈，多轮修正外观与几何细节；
> 2) **Velocity reweighting（动态重加权）**：基于图像平面渲染速度 `v_r` 对光度损失做像素级增权，强化动态 actor 的监督；
> 3) **Polynomial motion modeling（多项式运动，默认 ℓ=1）**：从常速度扩展到常加速度，以更好拟合加速/刹车/转向等复杂行为。

### 5.1 目标与门禁（成功标准）

- 目标：在不引入任何显式 3D 标注/动态掩码的前提下，提高（1）细节真实感（纹理/边缘/遮挡）、（2）动态 actor 的流与外观质量，并保持推理效率仍可用于快速重建与仿真。
- 门禁（建议至少在 tiny clip 上对比验证）：
  - 门禁 A（迭代 refinement 有效）：`N=3` 相比 `N=1`，渲染图更锐利、颜色更一致，PSNR/SSIM 不下降（通常上升）。
  - 门禁 B（动态重加权有效）：开启 `L_rgb_vw` 后，动态区域误差更低且更稳定；`|v_r|` 不出现大面积恒为 10 的饱和（已做 clip）。
  - 门禁 C（多项式运动有效）：`ℓ=1` 相比 `ℓ=0`，在加减速/转弯片段中动态一致性更好；若无收益需在记录中写明并允许回退。

### 5.2 总损失与“尽可能静止”正则（正文 3.3 Eq.(3)(4)）

- 总损失：
  - `L = L_recon + λ_vel L_vel`
  - `L_recon = λ_rgb L_rgb + λ_SSIM L_SSIM + λ_depth L_depth`
- 速度正则（as static as possible）：`L_vel = mean(||V||)`（对 `vdim=3(ℓ+1)` 的每个 3D block 求范数后平均）。

> 说明：本阶段引入的动态重加权只会“替换/改造” `L_rgb`，其余 `L_SSIM/L_depth/L_vel` 的定义不变。

### 5.3 运动增强 1：多项式运动模型（补充材料 A.1 Eq.(2)）

补充材料将速度参数化为多项式（degree 为 `ℓ`），并用其积分形式推进位置。对第 i 个高斯，从捕获时间 `t_i` 推进到目标时间 `t'`：
...
- 默认启用 `ℓ=1`（常加速度假设），因此 `vdim = 3(ℓ+1) = 6`，并使用展开式：
  - `p_i(t') = p_i + v_{i,0}*(t'-t_i) + v_{i,1}*(t'^2 - t_i^2)/2`
- `V` 的张量布局约定为：`V = concat([v0(3), v1(3), ..., vℓ(3)], dim=-1)`。
- 渲染/可视化/损失中凡涉及“时间推进”，一律使用该多项式推进（不要只改训练或只改可视化）。

### 5.4 运动增强 2：动态重加权（Velocity reweighting，补充材料 A.1）

动机：动态 actor 像素占比小，直接对整幅图平均会稀释动态监督；因此对光度损失做像素级重加权。

补充材料给出的定义：

- `L_rgb_vw = (1 + ||sg(v_r)||) · L_rgb`
  - `sg(·)` 表示 stop-gradient（权重不反传）
  - `v_r` 为图像平面渲染速度（rendered velocity in image space）
- 稳定性：对 `||v_r||` 做裁剪（补充材料给出 `[0,10]`）：
  - `w = 1 + stop_grad(clip(||v_r||, 0, 10))`
  - 用 `w` 替代 `L_rgb` 的像素权重图（对 `L_SSIM/L_depth` 不加权）
- 有效区域：只在 `valid_mask` 上应用（阶段4建议使用 `alpha > τ`），避免无效像素引入异常梯度。

### 5.5 真实感增强：迭代 refinement（正文 3.4 + 补充材料 A.2/A.3 Algorithm 2）

动机：Flux4D-base 单次前馈容易欠拟合细节；迭代 refinement 借助渲染监督产生的 3D 梯度反馈，逐步纠正颜色不一致与细节模糊（正文 3.4）。

补充材料给出的关键口径（必须对齐）：

- refinement 网络 `r_θ` 使用与 `f_θ` 同类的稀疏 3D U-Net 架构（补充材料 A.2）。
- 迭代次数固定 `N=3`（补充材料 A.2）。
- `step==0`：用 `f_θ(G_init, T)` 输出 `ΔG^(0), V`。
- `step>0`：用 `r_θ(G^(k), T, ∇G^(k))` 输出 `ΔG^(k)`；其中 `∇G^(k)` 为上一步迭代产生的梯度反馈（补充材料描述为 detached gradients）。
- 最终高斯为残差和形式（补充材料 A.2）：
  - `G = G_init + Σ_{j=0..N-1} ΔG^(j)`

补充材料 Algorithm 2 的训练闭环语义（实现时保持一致）：

1) 初始化：`G = G_init`，`G_grad = None`  
2) for `step in range(N)`：
   - if `step == 0`：`G_delta, V = F_Network(G, T)`
   - else：`G_delta = R_Network(G, T, G_grad)`
   - `G = G + G_delta`
   - `L = compute_loss(G, T, V)`（使用正文 Eq.(3)(4)，并叠加本阶段的 `L_rgb_vw` 与多项式推进）
   - `L.backward()`
   - `G_grad = detach(G.grad)`（作为下一轮输入；需避免把上一轮的计算图带入下一轮）

工程输入维度约定（避免实现者临时拍脑袋）：

- `R_Network` 的点级输入建议为：`concat([G(14), T(1), G_grad(14)], dim=-1)` → 每点 29 维；体素化与 U-Net `in_channels` 相应调整。
- `V` 的预测：按补充材料伪代码，仅在 `step==0` 预测一次；后续 refinement 默认只更新 `ΔG`（如需每轮更新 `V`，必须做消融并记录收益/代价）。

### 5.6 与阶段4的接口（必须复用阶段4产物，避免重复实现）

- 阶段5依赖阶段4的 `Rendered Velocity` 输出构造权重图：
  - 输入：`|v_r|`、`valid_mask`（优先由 `alpha` 推导）。
  - 输出：`weight_map w`（stop-grad + clip）。
- 迭代 refinement 的监督仍来自“渲染 RGB/Depth 与真实观测”的光度/深度损失；因此阶段5训练循环中渲染接口必须与阶段4一致（同一套时间推进与激活规则）。

### 5.7 额外项（非本阶段训练闭环刚需，可后续单独实现）

- 动态聚类（补充材料 Algorithm 3）：按 `||V||` 阈值筛出动态高斯并用 DBSCAN 聚类，属于“可控仿真/实例级编辑”能力，不是本阶段训练闭环必选项；可在后续 simulation tools 阶段实现。

## 阶段6：全量训练与评测

— 实现方案（严格对齐正文 4.1 的 PandaSet 设置 + 指标口径；补齐 Scene Flow GT 生成方案，避免评测“看似能跑但不对”）

> 本阶段目标是：在 PandaSet 上完成跨场景全量训练，并用论文相同的片段采样与评测协议输出可对齐的定量指标与可视化结果。
> **强约束：** 必须把“窗口切片、帧索引、时间归一化、dynamic-only 口径、Scene Flow GT 口径”全部写死在配置与评测代码中，避免实现时临时拍脑袋导致隐性 bug。

### 6.1 目标与交付物（必须产出）

**训练：**

- 数据集：PandaSet 动态场景（论文：103 个动态场景；前视相机 + 360° LiDAR，10Hz）。
- 划分：按论文做法留出 10 个场景做验证，其余训练。
  - 实现要求：用固定 seed 从 log 列表抽取 `val_log_ids`，并固化到 `data/metadata/pandaset_*.pkl`，保证可复现。

**评测（对齐论文）：**

- NVS/重建：输出 PSNR / SSIM / Depth RMSE，**同时报告 full-image 与 dynamic-only** 两套口径。
- Scene Flow：输出 EPE3D / Acc5 / Acc10 / θϵ / EPE-3way（BS/FS/FD）/ 分桶归一化 EPE；并记录推理速度。
- 可视化：至少导出 3 个典型片段的 4D 重建视频（插值 + 未来预测），包含 RGB / Depth / `|v_r|`（或 flow HSV）对比。

**落盘产物（强制）：**

- `assets/vis/stage6_eval/<run_id>/`：视频/关键帧/可视化图。
- `assets/metrics/stage6_eval/<run_id>.json`：所有指标（含 mean/std、full/dynamic、插值/未来分组、Scene Flow 全套指标与样本数）。
- `docs/开发记录/复现记录.md`：记录本次全量训练的配置、指标、结论与下一步（每次全量训练/评测都必须写）。

### 6.2 数据划分与片段采样（严格对齐论文，避免 off-by-one）

**传感器：**

- 仅用 `front_camera`（1080p）与 360° LiDAR（10Hz）。

**验证片段（论文口径，必须一致）：**

- 每个验证 log 全时长：8 秒（10Hz → 80 帧，索引 `0..79`）。
- **评测窗口长度：16 帧**（索引 `0..15`）。
  - 说明：论文文字写“1.5s short window”，但给出的评测帧集合包含 `11..15`，因此窗口必须覆盖 `0..15` 共 16 帧；实现必须以帧集合为准，避免少取 1 帧导致未来评测越界/缺帧。
- 每 20 帧采样一个新片段，每 log 产生 4 个不重叠评测片段：
  - `start ∈ {0, 20, 40, 60}`
  - 片段帧范围：`[start, start+15]`

**NVS 评测帧集合（逐片段）：**

- 输入帧（observed / input）：`{start+0, start+2, start+4, start+6, start+8, start+10}`
- 插值评测帧（interpolation）：`{start+1, start+3, start+5, start+7, start+9}`
- 未来评测帧（future）：`{start+11, start+12, start+13, start+14, start+15}`

**时间戳口径（避免 dt bug）：**

- clip 内对 LiDAR timestamp 做 min-max 归一化：`t_norm ∈ [0, 1]`（阶段3已约定）。
- 统一约定两个时间间隔（仅用于日志与 debug，不允许混用）：
  - `Δt_norm = t_norm[j] - t_norm[i]`
  - `Δt_sec = timestamp[j] - timestamp[i]`
- 任何从“运动参数/速度”转换到**米制位移**的地方，一律基于 `Δt_norm`（与阶段4/5的推进一致）。如需使用秒制（m/s），必须先显式做单位换算并在代码里写断言。

### 6.3 重建指标（PSNR/SSIM/Depth RMSE）口径（full + dynamic）

**渲染与对齐：**

- 对每个评测帧 `k`，用该帧相机位姿 `T_cam_world(k)` 渲染 `RGB_pred(k), Depth_pred(k)`，并与 `RGB_gt(k)`（以及该帧 LiDAR 投影深度）对齐比较。

**RGB 指标：**

- `RGB` 统一在 `[0,1]` 浮点域计算（实现时必须 clamp 到 `[0,1]`，避免溢出导致 PSNR 虚高/虚低）。
- PSNR（Peak Signal-to-Noise Ratio，单位 dB）：设 `MAX=1`（因为 RGB 已归一化到 `[0,1]`），则
  - `MSE = mean((RGB_pred - RGB_gt)^2)`（对 `H×W×3` 全部元素取均值）
  - `PSNR = 10 * log10(MAX^2 / MSE) = -10 * log10(MSE)`（`MAX=1`）
  - **dynamic-only** 时的 MSE：只在 `dynamic_mask=True` 的像素上取均值，且仍需对 **3 个通道一起平均**；
    不能只除以像素数（会引入常数偏移 `10·log10(3)≈4.77dB`，导致 full/dynamic 两套口径不可比）。
- SSIM：使用固定实现（如 kornia），并把输入范围与窗口参数写入配置，避免“同名 SSIM 但实现不同”导致不可比。

**Depth RMSE：**

- 深度 GT：由 LiDAR 投影得到的稀疏深度（单位：米）。
- 计算 mask：`valid_depth_mask = depth_gt > 0`，仅在有效像素上计算 RMSE。

**dynamic-only（必须实现以对齐论文）：**

- dynamic-only 的 RGB/Depth 指标在 `dynamic_mask(k)` 上计算（见 6.4），并与 full-image 指标一并汇总输出（分别对插值/未来分组）。

### 6.4 dynamic mask 生成（采用 box/track + LiDAR 点投影，复用 Scene Flow 赋值逻辑）

动机：论文要求报告“全图 + 动态移动区域”；若用阈值化 `|v_r|`，阈值敏感且口径漂移。本复现采用更稳定、可复现的 annotation 驱动口径。

**动态对象判定（逐帧对 k→k+1）：**

- 以 cuboid track 为单位，比较同一 `track_id` 在 `k` 与 `k+1` 的 3D 姿态变化，满足任一则判为动态：
  - 平移阈值：`||Δt_world|| > 0.05m`
  - 旋转阈值：`Δyaw > 1°`
- 阈值必须写入配置（默认如上），且实现时必须做平滑/去抖（例如对 3 帧做多数表决），避免标注抖动导致“静止物体被误判动态”。

**生成 dynamic_mask（对每个评测帧 k）：**

1) 对该帧 LiDAR 点做“点→实例”分配（与 6.5 的 Scene Flow GT 分配逻辑复用同一套实现，避免口径分叉）。
2) 取属于“动态实例”的点集合，投影到 `front_camera` 得到像素 `uv`。
3) 在图像上 rasterize 成稀疏 mask，并做形态学膨胀（默认半径 5px，可配），得到 `dynamic_mask(k)`。
4) 越界点必须丢弃（避免数组越界）；mask 必须与 `RGB_gt` 同尺寸。

### 6.5 Scene Flow：GT 生成与评测口径（cuboid + pose，强制写死）

**评测点集合（对齐论文描述）：**

- 仅评测 **相机 FoV 内** 的 LiDAR 点（遵循 STORM [60] 的做法）：
  - 对 LiDAR 点 `p_world` 用 `T_cam_world(k)` 投影到像素；
  - 满足 `z_cam > 0` 且 `0 ≤ u < W, 0 ≤ v < H` 才计入评测。

**Scene Flow GT（由标注构造，避免“没有 GT 还硬评测”的隐性 bug）：**

对每个相邻帧对 `(k, k+1)`，对帧 `k` 的每个 LiDAR 点 `p_world(k)`：

1) 点→cuboid 分配：若点落在任一 cuboid 内，分配给该 cuboid。
   - 点在 box 内判定：`p_obj = inv(T_world_obj(k)) ⊗ p_world(k)` 后按 box 尺寸做 axis-aligned 范围判断。
   - 多重命中时：选体积更小者；若体积相同选 `||p_obj||` 更小者（规则必须写死，避免随机性）。
2) 若点属于某实例 `id`：
   - 找到帧 `k+1` 同 `track_id` 的 cuboid；若缺失则该点 `valid_gt=False`。
   - 用 cuboid 的刚体变换把点从 `k` 的实例坐标系搬运到 `k+1`：
     - `p_obj = inv(T_world_obj(k)) ⊗ p_world(k)`
     - `p_world_hat(k+1) = T_world_obj(k+1) ⊗ p_obj`
3) 若点不属于任何 cuboid（背景-静态，BS）：
   - 视为世界静止：`p_world_hat(k+1) = p_world(k)`，因此 `flow_gt_world = 0`。
4) `flow_gt_world = p_world_hat(k+1) - p_world(k)`（单位：米）。

**Scene Flow 预测（与阶段4/5的运动模型严格一致，避免 dt/坐标系 bug）：**

- 预测运动参数必须使用 `world` 语义（阶段3已约定将网络输出从 `ego0` 旋转回 `world`）。
- 对帧对 `(k, k+1)` 的预测 flow 定义为“用相同运动模型推进后的位移”：
  - 设 `t0 = t_norm[k]`，`t1 = t_norm[k+1]`，且该点/高斯的捕获时间为 `t_i = t0`。
  - 若使用常速度（`ℓ=0`，`vdim=3`）：`flow_pred_world = v_world * (t1 - t0)`
  - 若使用多项式运动（`ℓ>=1`，`vdim=3(ℓ+1)`）：
    - `flow_pred_world = Σ_{j=0..ℓ} v_{j,world} * (t1^{j+1} - t0^{j+1}) / (j+1)`
  - 评测必须只在 `FoV_mask & valid_gt_mask` 上计算。

**Scene Flow 指标定义（写死，避免口径漂移）：**

- `EPE3D = mean(||flow_pred - flow_gt||_2)`
- `Acc5 = mean(||e||_2 ≤ 0.05m)`，`Acc10 = mean(||e||_2 ≤ 0.10m)`（误差 ≤ 5/10cm 的点比例）
- `θϵ`（弧度角误差）：`arccos( dot(fp, fg) / (||fp||·||fg|| + eps) )` 对点取 mean；`eps` 写死（如 `1e-6`）。
- `EPE-3way`：按点类别分组均值（每组必须同时输出样本数，避免“空组均值=NaN” bug）：
  - BS：背景-静态（未命中任何 cuboid）
  - FS：前景-静态（命中 cuboid，但实例未被判为动态）
  - FD：前景-动态（命中 cuboid，且实例被判为动态）
- 分桶归一化 EPE（Bucketed normalized EPE）：
  - 先定义 `nEPE = ||e||_2 / max(||flow_gt||_2, 0.05m)`（分母下界写死，避免 `flow_gt≈0` 导致数值爆炸）。
  - 桶定义（默认与论文 WOD 的语义桶对齐）：`background / vehicle / pedestrian / cyclist / other`，按 cuboid 类别映射（映射表必须写入配置，并在预处理阶段打印全量唯一类别进行人工核对）。
  - 每桶输出：`mean(nEPE)` + `count`。

**强制 sanity checks（任一不通过就判为实现有 bug）：**

- 对 BS（背景-静态）：
  - `mean(||flow_gt||)` 必须接近 0（否则 GT 构造/坐标系有错）
  - `mean(||flow_pred||)` 随训练应下降（否则速度正则/坐标系/时间推进链路有错）
- 对 FoV 过滤：
  - FoV 内点比例应稳定且非 0（否则投影/外参/分辨率有错）

### 6.6 训练与评测运行策略（避免资源与复现实验漂移）

**运行入口（本仓库已实现，避免“文档说得通但代码缺口径”）：**

- 全量训练：`scripts/train_stage6.py`（内部调用 `src/flux4d/engine/stage6.py:train_stage6_full`）
- 评测（NVS + Scene Flow）：`scripts/eval_stage6.py`（内部调用 `src/flux4d/engine/stage6.py:eval_stage6`）
- 依赖：`torch + gsplat(+spconv)`；dynamic mask / Scene Flow 需要能读取 cuboids 标注（PandaSet 为 `pandas.DataFrame` pickle，需 `pandas`）。

**推荐命令（示例）：**

- 生成对齐论文的索引（若尚未生成）：`make pandaset-index`
- 全量训练（建议为每次 run 单独建目录，避免覆盖）：  
...
- 评测：  
  `python scripts/eval_stage6.py --config configs/flux4d.py --index-path data/metadata/pandaset_full_clips.pkl --ckpt assets/vis/stage6_train/run_001/ckpt_last.pt --out-dir assets/vis/stage6_eval/run_001 --save-renders`

**训练迭代数（论文参考）：**

- 论文：30,000 iters（4× L40S 48G，约 2 天）。
- 本复现：`train.total_iters=30000` 作为默认；单卡调试允许降低迭代数，但必须在记录中注明“非最终指标”。

**资源约束（24G 调试 vs A100 正式）：**

- 24G（5090）仅用于功能/门禁验证：减小 `num_views`、必要时启用梯度累积与混合精度。
- A100 上跑正式全量训练：恢复论文视图数/分辨率口径；最终指标禁止使用“为了跑通而下采样”的设置。

**评测频率与保存：**

- 每 `eval.every_n_iters` 做一次验证，至少保存 `ckpt_best` 与 `ckpt_last`。
- 指标汇总必须拆分：
  - 插值 vs 未来；
  - full-image vs dynamic-only；
  - Scene Flow（相邻帧对）单独汇总；
  避免把不同任务/不同口径混为一个均值造成误读。

### 6.7 本阶段验收清单（实现者按此自检）

- 文档中已明确：16 帧窗口、`start={0,20,40,60}`、输入/插值/未来帧集合。
- 时间口径无歧义：motion/flow/Scene Flow 都基于 `t_norm`，且代码有断言防止混用秒制。
- dynamic-only mask 使用 box/track + LiDAR 投影生成，且与 Scene Flow 的点分配复用同一套逻辑。
- Scene Flow GT 由 cuboid + pose 构造，且 BS 的 `flow_gt≈0` sanity check 通过。
- 输出指标包含：full + dynamic、插值 + 未来、Scene Flow 全套指标、每组样本数与推理速度。
