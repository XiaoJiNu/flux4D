...
- 目标：在任意目标时间 `t'`（插值/未来）下，对预测高斯进行时间推进并渲染 `RGB/Depth`；同时在图像平面渲染用于可视化与动态重加权的 `Rendered Velocity/Rendered Flow`。
- 成功标准（门禁）：
  - 相邻时间渲染的 `RGB` 变化与 `Rendered Flow/Velocity` 方向一致（动态目标朝运动方向，背景接近 0）。
  - `Rendered Velocity` 的幅值分布稳定，按补充材料做 `clip [0,10]` 后不出现大面积饱和。
  - 插值帧（例如 1、3、5、7、9）渲染稳定，不出现明显跳变；未来帧（例如 11–15）可正常输出且趋势合理（不要求完全准确）。
...
- `Rendered Velocity v_r(t')`：**图像平面的渲染速度**（2 通道，单位：pixels / Δt）。
- `|v_r|(t')`：速度幅值图（用于重加权与调试）。
- `valid_mask(t')`：可见性/权重掩码（建议用累计 alpha 或者 weight sum）。

> 说明：论文正文提到“render the flow in the image plane”，补充材料 A.1 明确用于动态重加权的量是 `v_r`（rendered velocity in image space），并对其做 `clip [0,10]` 后参与损失加权。
...
- 连续渲染 `t0..tN` 合成视频，检查动态目标边缘是否随时间一致推进。
...
本复现对 `v_r` 给出一个**可实现且可验证**的严格定义（用于阶段4/5一致性）：

1) 选定一个时间步长 `Δt`（建议：用相邻采样帧在归一化时间下的差值，如 `t[k+1]-t[k]`；调试时也可固定一个常数，例如 `Δt=1/clip_len_frames`）。
2) 对每个高斯，在同一相机下投影其在 `t'` 与 `t'+Δt` 的像素坐标：
   - `u_i = Π(p_world(t'))`
   - `u'_i = Π(p_world(t'+Δt))`
3) 定义该高斯的图像平面速度（2D）：
   - `v_r,i(t') = (u'_i - u_i) / Δt`（单位：pixels / Δt）
4) 将 `v_r,i(t')` 作为“每个高斯的属性”，用与 RGB 相同的 splat/alpha 权重在图像上做加权合成，得到每像素的 `v_r(t')`（2 通道）与 `valid_mask(t')`。
5) 速度幅值：
   - `|v_r|(t') = sqrt(v_r_x^2 + v_r_y^2)`，并按补充材料建议用于训练时 `clip(|v_r|, [0,10])`（或对向量分量做等价裁剪/缩放）。

> 兼容“Rendered Flow”表述：若需要像素位移形式的 `Rendered Flow(t'→t'+Δt)`，可直接定义为 `flow = v_r * Δt`（单位：pixels），用于可视化检查位移方向与大小。
...
- 文件：`src/flux4d/render/flux4d_renderer.py`
  ...
  - `render_rgb_depth(gaussians, camera) -> (rgb, depth)`
  - `render_flow_map(gaussians, velocities, camera, t_src, t_tgt) -> flow`
    - 实现上建议输出 `v_r` + `|v_r|` + `valid_mask`，并在配置中定义 `Δt` 与 clip 范围。
- 文件：`tools/vis/vis_flow.py`
  - 输出：`RGB(t')`、`|v_r|(t')`、`v_r` 可视化（箭头/HSV）以及相邻帧差分图，作为阶段4门禁材料。
